...
void Game::CreateWorld() {
	...
	*/
	...
	/* no underworld levels, then no gates there
...
int ARegionList::GetRegType( ARegion *pReg, const int odd ) {
	...
	while (1) {
	  ...
	  // Surface region
	  int r = getrandom(20);
	  if (r==0) newterrain=R_OCEAN;
	  if (r==1) newterrain=R_PLAIN;
	  if (r==2) newterrain=R_LAKE;
	  if (r==3) newterrain=R_PLAIN;
	  if (r==4) newterrain=R_TUNDRA;
	  if (r==5) newterrain=R_RICHTUNDRA;
	  if (r==6) newterrain=R_TUNDRA;
	  if (r==7) newterrain=R_DESERT;
	  if (r==8) newterrain=R_OASIS;
	  if (r==9) newterrain=R_DESERT;
	  if (r==10) newterrain=R_FOREST;
	  if (r==11) newterrain=R_MYSTFOREST;
	  if (r==12) newterrain=R_FOREST;
	  if (r==13) newterrain=R_HILL;
	  if (r==14) newterrain=R_RICHHILL;
	  if (r==15) newterrain=R_HILL;
	  if (r==16) newterrain=R_MOUNTAIN;
	  if (r==17) newterrain=R_RICHMOUNTAIN;
