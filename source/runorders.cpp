...
int Game::TaxCheck(ARegion *pReg, Faction *pFac) {
	if (Globals->FACTION_LIMIT_TYPE == GameDefs::FACLIM_FACTION_TYPES) {
		...
		if (pFac->war_regions_num >= AllowedTaxes(pFac)) {
			//
			// Can't tax here.
			//
			return 0;
			...
			if (pReg->town) {
				switch (pReg->town->TownType()) {
				case TOWN_CITY: 
					pFac->war_regions_num+=Globals->CITY_TAX_COST;
					break;
				case TOWN_TOWN:
					pFac->war_regions_num+=Globals->TOWN_TAX_COST;
					break;
				case TOWN_VILLAGE:
					pFac->war_regions_num+=Globals->VILLAGE_TAX_COST;
					break;
				}
			} else pFac->war_regions_num++;
			return 1;
		}
	} else {
		//
	...
	if (Globals->FACTION_LIMIT_TYPE == GameDefs::FACLIM_FACTION_TYPES) {
		...
		if (pFac->trade_regions_num >= AllowedTrades(pFac)) {
			...
			if (pReg->town) {
				switch (pReg->town->TownType()) {
				case TOWN_CITY: 
					pFac->trade_regions_num+=Globals->CITY_TRADE_COST;
					break;
				case TOWN_TOWN:
					pFac->trade_regions_num+=Globals->TOWN_TRADE_COST;
					break;
				case TOWN_VILLAGE:
					pFac->trade_regions_num+=Globals->VILLAGE_TRADE_COST;
					break;
				}
			} else pFac->trade_regions_num++;
			ARegionPtr * y = new ARegionPtr;
			y->ptr = pReg;
...
AList * Game::CanSeeSteal(ARegion * r,Unit * u) {
	...
	forlist(&factions) {
		...
		if (r->Present(f)) {
			if (f->CanSee(r,u, Globals->SKILL_PRACTISE_AMOUNT > 0,true)) {
...
void Game::Do1Steal(ARegion * r,Object * o,Unit * u) {
	...
	if (tar->type == U_GUARD || tar->type == U_GUARDMAGE) {
		u->Error("STEAL: Cannot steal from city guards.");
		return;
	}
